...
| # | Idea | Complexity | Notes | Your Notes |
|---|------|------------|-------|------------|
| 160 | Staff forms: shared table layout, restyleable from one place | Medium | Reference implementation exists: review draft edit page (2026-09-26) | Roll the label/value table layout from `staff/reviews/review-drafts/form.twig` out to other staff forms (78 staff templates contain a `<form>`, 29 use the old `row mb-3` + `col-form-label` grid). **Build the shared pieces first, then migrate forms one at a time** - the aim is that a later restyle is a one-file change, not another 29-form pass. (1) **Styles in the stylesheet, not the template:** move the inline `<style>` block out of the review draft form into `public/staff-b5/custom.css` as a single class (e.g. `.staff-form-table`) and bump the `?v=` in `theme/staff-b5/base.twig`. Drive the look from CSS custom properties on that class - label column background (currently `#e7f1ff`), label width (180px), cell padding (`0.9rem 1.1rem`), max width - so colour, spacing or width changes touch one line. (2) **Form controls:** style inputs and selects via `.staff-form-table .form-control` / `.form-select` rules in the same place, so control styling can be changed without editing forms. Add one class for help text (e.g. `.staff-form-help`), and make it the only muted text in a form - that was the rule on the review draft page. (3) **Wrapper and actions in a shared template:** a Twig embed (e.g. `ui/components/form/table-form.twig` with `rows` and `actions` blocks) that owns the table, the width wrapper and the submit area, so the submit button's style and position (below the table, inside a final row, right-aligned, sticky) is decided in one place. Forms supply rows and, if needed, override `actions`. Note the existing `ui/components/form/submit.twig` macro uses `col-md-offset-4`, a Bootstrap 3 class that does nothing under B5 - fix or replace it as part of this. (4) **Rows stay plain markup** (`<tr><th><label for=...></th><td>...</td></tr>`): Twig macros can't wrap a body, and a macro per field type would be more code than the rows themselves. (5) Keep field ids unchanged when migrating, since each form's `scripts.twig` sets values by id. Migrate the review draft form onto the shared pieces first as the proof, then the most-used forms. |
...
| 159 | Two cron commands don't log to the `cron` channel | Low | A few lines each; follows an existing pattern | Every line in `scripts/switchscores-cron` sends output to `/dev/null` by design - commands write their own log via `Log::channel('cron')` (the `cron` stack → `cron-daily`, i.e. `storage/logs/cron-YYYY-MM-DD.log`), which keeps the log path out of the cron file. Checked 2026-09-25: 20 of the 22 scheduled commands do this (e.g. `UpdateGameRanks.php:53`). **Two don't:** `app:fetch-gsc-page-snapshots` (`FetchGscPageSnapshots.php`, no logging at all) and `game:sync-steam-reviews` (`Game/SyncSteamReviews.php`, console output only via `$this->info`/`$this->warn`, so its success/failed counts vanish into `/dev/null`). Both are recent additions. Add the channel with the usual `*** signature ***` header line, and log the summary counts. Worth landing before the infrared-switch migration, since the first nights on a new box are when the cron log gets read. |
